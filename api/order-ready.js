...
import { getOrder, updateOrder, getPushSubscription } from "./redis.js";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const order = await getOrder(d, orderNumber);
    if (!order) return res.status(404).json({ error: "Order not found" });

    await updateOrder(d, orderNumber, { status: "ready" });

    const sub = await getPushSubscription(d, orderNumber);
    let pushed = false;
    ...
    if (sub) {
      try {
        await webPush.sendNotification(sub, JSON.stringify({
          ...
          orderNumber,
          url:         `/?orderReady=${orderNumber}&date=${d}`,
        }));
        pushed = true;
      } catch (e) {
