    ...
    <div className="text-cream max-w-4xl mx-auto">
      ...
      {adding && (
        <div className="bg-char-soft border border-marigold/30 rounded-2xl p-5 mb-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-display font-semibold text-marigold">Nouveau combo</h3>
      ...
      <div className="space-y-4">
        {combos.map((combo) => {
          ...
          return (
            <div key={combo.id} className="bg-char-soft rounded-2xl border border-cream/5 overflow-hidden">
              {editingId === combo.id ? (
                <div className="p-5">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="font-display font-semibold text-marigold">Modifier le combo</h3>
                    <button onClick={() => setEditingId(null)}><X size={18} className="text-mute/50" /></button>
                  </div>
                  <ComboForm
                    combo={editDraft}
                ...
                <div className="flex items-center gap-4 px-4 py-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      <h3 className="font-semibold text-cream text-sm truncate">{combo.name}</h3>
                      {combo.tag && (
                    ...
                    <p className="text-xs text-mute/60 truncate">
                      {[...(combo.items || []), ...(combo.beverages || []).map((b) => ({ name: b.name }))].map((i) => i.name).filter(Boolean).join(", ")}
                    </p>
                    <div className="flex items-center gap-3 mt-1">
                      <span className="font-mono text-xs font-semibold text-marigold">{totals.discounted}€</span>
                      <span className="font-mono text-[10px] text-mute/40 line-through">{totals.original}€</span>
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    <button onClick={() => startEdit(combo)} className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-cream/10 text-mute/60 hover:text-cream transition">
                      <Pencil size={14} />
                    </button>
                    <button onClick={() => deleteCombo(combo.id)} className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-chili/10 text-mute/60 hover:text-chili transition">
                      <Trash2 size={14} />
                    </button>
                  </div>
                </div>
      ...
      {combos.length === 0 && !adding && (
        <div className="text-center py-16 text-mute/40">
          ...
          <p className="text-xs mt-1">Créez votre premier combo en cliquant sur "Créer un combo".</p>
        </div>
      )}
    </div>
  );
...
// ─── Combo Form ───────────────────────────────────────────────────────────────
function ComboForm({ combo, setCombo, menuItems, onSave, onCancel, saveLabel, saving }) {
    ...
    setCombo((prev) => ({
      ...
      items: [
        ...(prev.items || []),
        {
          ...
          comboPrice: parseFloat((firstItem.price - (firstItem.discount || 0)).toFixed(2)),
        },
      ],
    }));
  };
    ...
    setCombo((prev) => {
      ...
      if (field === "menuItemId") {
        ...
        items[idx] = {
          ...
          comboPrice: parseFloat(((found?.price || 0) - (found?.discount || 0)).toFixed(2)),
        };
      } else {
  ...
  const removeMenuItem = (idx) =>
    setCombo((prev) => ({ ...prev, items: prev.items.filter((_, i) => i !== idx) }));

  // Beverage management
  const addBeverage = () =>
    setCombo((prev) => ({ ...prev, beverages: [...(prev.beverages || []), { ...EMPTY_BEVERAGE }] }));

  const updateBeverage = (idx, field, value) => {
    setCombo((prev) => {
      const beverages = [...(prev.beverages || [])];
      beverages[idx] = { ...beverages[idx], [field]: field === "name" ? value : parseFloat(value) || 0 };
      return { ...prev, beverages };
    });
  };

  const removeBeverage = (idx) =>
    setCombo((prev) => ({ ...prev, beverages: prev.beverages.filter((_, i) => i !== idx) }));
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="col-span-2">
          ...
          <input value={combo.name} onChange={set("name")} placeholder="Ex: Combo Poulet Royal" className="admin-input w-full" />
        </div>
        <div>
          <Label>Tag / badge</Label>
          <input value={combo.tag || ""} onChange={set("tag")} placeholder="Best-seller, Nouveau..." className="admin-input w-full" />
        </div>
        <div>
          <Label>Emoji (si pas d'image)</Label>
          <input value={combo.emoji || "🎉"} onChange={set("emoji")} placeholder="🎉" className="admin-input w-full" />
        </div>
      </div>
      ...
      <div>
        ...
        {imageTab === "url" ? (
          ...
          <>
            ...
            >
              <Image size={16} />
              {combo.image?.startsWith("data:") ? "Image chargée ✓" : "Cliquez pour choisir une image"}
            </button>
          </>
        )}
        {combo.image && (
          <div className="mt-2 flex items-center gap-2">
            <img src={combo.image} alt="preview" className="w-16 h-16 rounded-xl object-cover border border-cream/10"
              onError={(e) => { e.target.style.display = "none"; }} />
            <button onClick={() => setCombo((p) => ({ ...p, image: "" }))} className="text-xs text-chili hover:underline">
              Supprimer
            </button>
          </div>
        )}
      ...
      <div>
        ...
        {(combo.items || []).length === 0 && (
          <p className="text-xs text-mute/40 italic mb-2">Aucun plat ajouté. Cliquez sur "Ajouter un plat".</p>
        )}
          ...
          {(combo.items || []).map((item, idx) => (
            <div key={idx} className="bg-char rounded-xl p-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <p className="text-[10px] text-mute/50 mb-1">Prix original (auto)</p>
                  <div className="admin-input text-xs text-mute/60 font-mono">
                    {item.originalPrice?.toFixed(2)}€
                  </div>
                </div>
                <div>
                  <p className="text-[10px] text-mute/50 mb-1">Prix dans le combo (€)</p>
                  <input
                    type="number"
                    step="0.10"
                    min="0"
                    value={item.comboPrice}
                    onChange={(e) => updateMenuItem(idx, "comboPrice", e.target.value)}
              ...
              {item.comboPrice < item.originalPrice && (
                <p className="text-[10px] text-herb mt-1.5">
                  Remise: -{(item.originalPrice - item.comboPrice).toFixed(2)}€ sur cet article
                </p>
              )}
            </div>
          ))}
            ...
            <div key={idx} className="bg-char rounded-xl p-3">
              <div className="flex items-center gap-2 mb-2">
                ...
                <input
                  ...
                  className="admin-input flex-1 text-xs"
                />
                <button onClick={() => removeBeverage(idx)} className="text-mute/40 hover:text-chili shrink-0">
                  <X size={15} />
                </button>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <p className="text-[10px] text-mute/50 mb-1">Prix original (€)</p>
                  <input
                    type="number"
                    step="0.10"
                    min="0"
                    value={bev.originalPrice}
                    onChange={(e) => updateBeverage(idx, "originalPrice", e.target.value)}
                    className="admin-input w-full text-xs"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => updateBeverage(idx, "comboPrice", e.target.value)}
                    className="admin-input w-full text-xs"
                  />
                </div>
              </div>
              {bev.comboPrice < bev.originalPrice && (
                <p className="text-[10px] text-herb mt-1.5">
                  Remise: -{(bev.originalPrice - bev.comboPrice).toFixed(2)}€ sur cette boisson
                </p>
              )}
            </div>
          ))}
      ...
      {(combo.items?.length > 0 || combo.beverages?.length > 0) && (
        <div className="bg-char rounded-2xl p-4 border border-marigold/20">
          <p className="text-xs font-semibold text-marigold mb-2">Aperçu des prix</p>
          <div className="space-y-1.5">
            {[...(combo.items || []), ...(combo.beverages || []).map((b) => ({ name: b.name || "Boisson", originalPrice: b.originalPrice, comboPrice: b.comboPrice }))].map((item, i) => (
              <div key={i} className="flex justify-between text-xs">
                <span className="text-mute/70">{item.name || "—"}</span>
                <span className="font-mono">
                  <span className="text-marigold">{(item.comboPrice || 0).toFixed(2)}€</span>
                  <span className="text-mute/30 line-through ml-1.5">{(item.originalPrice || 0).toFixed(2)}€</span>
                </span>
              </div>
            ))}
            <div className="pt-2 border-t border-cream/10 flex justify-between">
              <span className="text-sm font-semibold text-cream">Total combo</span>
              <span className="font-mono font-bold text-marigold">
                {totalCombo.toFixed(2)}€
                <span className="text-mute/30 line-through ml-1.5 font-normal text-xs">{totalOriginal.toFixed(2)}€</span>
              </span>
            </div>
