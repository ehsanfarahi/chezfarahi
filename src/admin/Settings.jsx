...
export default function Settings({ token }) {
  ...
  useEffect(() => {
    fetch(`${API_URL}/api/admin/business`, {
        ...
        <div className="space-y-2">
          {data.locations.map((loc, idx) => (
            <div key={idx} className="flex items-center gap-2">
              ...
              />
              <button
                onClick={() => updateField(["locations"], data.locations.filter((_, i) => i !== idx))}
        ...
        <div className="space-y-2">
          {data.recommendations.map((rec, idx) => (
            <div key={idx} className="flex items-center gap-2">
              ...
              />
              <button
                onClick={() => updateField(["recommendations"], data.recommendations.filter((_, i) => i !== idx))}
                className="text-mute/40 hover:text-chili shrink-0"
              >
                <Trash2 size={15} />
              </button>
