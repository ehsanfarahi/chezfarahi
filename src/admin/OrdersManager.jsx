...
export default function OrdersManager({ token }) {
  ...
  // ─── Mark ready + send push ────────────────────────────────────────────────
  const markReady = async (order) => {
    const orderNumber = order.orderNumber;
    setMarkingReady(order.orderId);

    // 1. Update local localStorage status
    ...
    // 3. Auto-open announce overlay so you can call out the number
  ...
  const displayed =
    filter === "pending" ? pending : filter === "done" ? done : orders;
        ...
        {displayed.map((order) => {
          ...
          return (
            ...
            >
              ...
              >
                {/* Prominent 3-digit order number */}
                <div
                  className={`font-display font-black text-2xl w-14 text-center shrink-0 ${
  ...
  return (
    <div className="fixed inset-0 z-50 bg-char/95 backdrop-blur-sm flex flex-col items-center justify-center text-center px-8">
      ...
      <p className="font-display text-2xl font-bold text-cream mb-2">
        Numéro{" "}
        <span className="text-marigold">{orderNumber}</span> — Prête !
      </p>
