...
export default function OrderQRModal({ cart, open, onClose, onConfirmed, onOrderReady }) {
  ...
  async function createOrder() {
    ...
    try {
      ...
      // 2. Build full order object including the server-assigned number
      const newOrder = {
        ...buildOrder(cart),
        orderNumber: num,
      };
      ...
      // 4. Save pending order to localStorage for global watcher in App.jsx
      localStorage.setItem(
        "camion_pending_order",
        JSON.stringify({ orderNumber: num, status: "pending" })
      );

      // 5. Start polling for status every 3 seconds
