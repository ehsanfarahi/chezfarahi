...
import MenuDuJourBlackboard from "./MenuDuJourBlackboard";

export default function MenuDuJourModal({ menu, open, onClose, onAdd }) {
  if (!open || !menu) return null;

  const uiStyle = menu.displayOptions?.uiStyle || "modern";

  if (uiStyle === "blackboard") {
    return (
      <MenuDuJourBlackboard
        menu={menu}
        open={open}
        onClose={onClose}
        onAdd={onAdd}
  ...
  return (
    <ModernModal menu={menu} open={open} onClose={onClose} onAdd={onAdd} />
  );
}

function ModernModal({ menu, open, onClose, onAdd }) {
  ...
  // Read display options - default everything to true if not set 
  const opts = {
    showItemPrices: true,
    showPromoTags: true,
    showImage: true,
  ...
  const allItems = [
    ...
    ...(menu.beverage?.included
      ? [
          {
            name: menu.beverage.name || "Boisson au choix",
            originalPrice: menu.beverage.price,
          },
        ]
      : []),
  ];
    ...
    <>
      ...
      <div className="fixed bottom-0 left-0 right-0 sm:inset-0 sm:flex sm:items-center sm:justify-center z-50">
        ...
        >
          ...
          >
            {/* Savings stamp - only if showPromoTags is true */}
            {opts.showPromoTags && savingsPct > 0 && (
              <div
            ...
            {/* Image or emoji - only if showImage is true */}
            <div className="mb-3">
              {opts.showImage && menu.image ? (
                <img
                  src={menu.image}
                  alt={menu.title}
            ...
            {["top-4 left-4", "top-4 right-12", "top-4 left-12"].map((pos) => (
              <span
                key={pos}
                className={`absolute ${pos} w-1 h-1 rounded-full bg-marigold/30`}
              />
            ))}
            ...
            {menu.subtitle && (
              <p
                ...
                style={{ color: "rgba(242,169,59,0.75)" }}
              >
                {menu.subtitle}
              </p>
            )}
            ...
            <div className="flex items-center gap-2 mt-5">
              <div
                ...
                style={{ background: "rgba(242,169,59,0.2)" }}
              />
              <span
                style={{ color: "rgba(242,169,59,0.4)", fontSize: "0.5rem" }}
              >
                ◆
              </span>
              <div
          ...
          <div className="px-6 py-4 space-y-0.5">
            {allItems.map((item, i) => (
              ...
              >
                ...
                {/* Per-item price (toggleable) */}
                {opts.showItemPrices && item.originalPrice > 0 && (
                  <span
                    className="font-mono text-xs line-through ml-2 shrink-0"
          ...
          <div className="flex items-center gap-2 px-6">
            <div
              className="flex-1 h-px"
              style={{ background: "rgba(242,169,59,0.15)" }}
            />
            <span style={{ color: "rgba(242,169,59,0.3)", fontSize: "0.5rem" }}>
              ◆
            </span>
            <div
              className="flex-1 h-px"
          ...
          <div className="px-6 py-5 text-center">
            {opts.showItemPrices && menu.totalOriginal > menu.menuPrice && (
              <p
                className="font-mono text-sm line-through mb-1"
                style={{ color: "rgba(232,223,211,0.35)" }}
              >
                Valeur totale : {Number(menu.totalOriginal).toFixed(2)}€
              </p>
            )}

            {/* Menu price - always shown*/}
            <p
              className="font-display font-black mb-2"
              style={{ fontSize: "3rem", color: "#F2!93B", lineHeight: 1 }}
            >
            ...
            {/* Promo savings badge (toffleable) */}
            {opts.showPromoTags && parseFloat(savings) > 0 && (
              <div
