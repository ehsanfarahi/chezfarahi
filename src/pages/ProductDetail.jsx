...
import {
  ArrowLeft, Heart, Share2, Minus, Plus,
  Flame, Leaf, Star, Check, ChevronDown, Tag,
} from "lucide-react";
...
export default function ProductDetail({ onAdd }) {
  ...
  const { getItemById, loading } = useMenuData();
  ...
  useEffect(() => {
    ...
    const onTouchEnd = (e) => {
      ...
      if (startX < 40 && e.changedTouches[0].clientX - startX > 80) navigate(-1);
      startX = null;
    };
  ...
  const categoryEmoji = (cat) =>
    ({ Samosas: "🥟", Sandwichs: "🥪", "Hot-Dogs": "🌭", Frites: "🍟", Boissons: "🥤", Desserts: "🍮" }[cat] || "🍽️");
  ...
  return (
    <div className="min-h-screen bg-char text-cream pb-28">
      ...
      <div className={`fixed top-0 inset-x-0 z-40 flex items-center justify-between px-4 py-3 transition-all duration-300 ${
        scrolled ? "bg-char/90 backdrop-blur border-b border-cream/10 opacity-100" : "opacity-0 pointer-events-none"
      }`}>
        <button onClick={() => navigate(-1)} className="w-9 h-9 rounded-full bg-char-soft flex items-center justify-center">
          <ArrowLeft size={16} />
        </button>
      ...
      <div className="relative w-full h-[36vh] sm:h-[40vh] overflow-hidden">
        {product.image ? (
          ...
          <div className="w-full h-full bg-gradient-to-br from-marigold/30 via-char-soft to-chili/20 flex items-center justify-center">
            <span className="text-[5.5rem] drop-shadow-lg">
        ...
        <div className="absolute top-4 right-4 flex items-center gap-2">
          ...
          <button className="w-10 h-10 rounded-full bg-char/60 backdrop-blur-md flex items-center justify-center hover:bg-char/80 active:scale-95 transition">
      ...
      <div className="relative -mt-6 bg-char-soft rounded-t-3xl">
              ...
              <div className="flex items-center gap-1.5 mt-1.5">
                {product.rating && (
                  <>
                    ...
                    <span className="text-xs text-mute/60">({product.reviews || 0} avis)</span>
                  </>
                )}
                {product.spicy > 0 && (
              ...
              {[
                ...
                product.nutrition[key] ? (
                  <div key={key} className="bg-char rounded-xl px-2 py-3 text-center">
                    <p className="font-mono font-semibold text-sm">{product.nutrition[key]}</p>
                    <p className="text-[10px] uppercase tracking-wide text-mute/60 mt-0.5">{label}</p>
                  </div>
                ) : null
              )}
            </div>
        ...
        {product.prepTime && (
          <div className="px-5 pt-3 pb-2">
            <p className="text-xs text-mute/50 flex items-center gap-1.5">
              ⏱ Temps de préparation : <span className="text-mute font-medium">{product.prepTime} min</span>
            </p>
          </div>
        )}
...
//   if (!product) {
//     return <NoProductFound navigate={navigate} />;
//   }
...
// function NoProductFound({ navigate }) {
//   <div className="min-h-screen flex flex-col items-center justify-center bg-char text-cream gap-4">
//     <p className="text-mute">Produit introuvable.</p>
//     <button
//       onClick={() => navigate(-1)}
...
// function DetailHeader({ scrolled, product, navigate }) {
//   return (
//     <div
...
//       <ImageTopRight favorited={favorited} setFavorited={setFavorited} />

//       {/* Tags overlay, bottom-left of image */}
//       <ImageOverlayBottomLeft product={product} />
//     </div>
//   );
// }

// function ImageTopRight({ favorited, setFavorited }) {
//   return (
//     <div className="absolute top-4 right-4 flex items-center gap-2">
//       <button
...
// function ImageOverlayBottomLeft({ product }) {
//   return (
//     <div className="absolute bottom-5 left-4 flex gap-2">
//       {product.tag && (
//         <span className="text-[10px] uppercase tracking-wider font-semibold bg-marigold text-char px-2.5 py-1 rounded-full">
...
//       {/* Nutrition facts */}
//       <InfoCardNutritionalFacts product={product} />

//       {/* Promotion banner */}
//       {product.promo && <PromotionBanner product={product} />}
//       <div className="h-4" />
//     </div>
//   );
// }
...
// function InfoCardDescription({ product, showFullDesc, setShowFullDesc }) {
//   return (
//     <div className="px-5 pt-7 pb-2">
...
// function InfoCardNutritionalFacts({ product }) {
//   return (
//     <div className="px-5 pt-4 pb-2">
...
// function PromotionBanner({ product }) {
//   return (
//     <div className="mx-5 mt-4 mb-2 rounded-2xl bg-gradient-to-r from-chili to-chili-dark p-4 flex items-center gap-3 shadow-lg shadow-chili/20">
//       <div className="w-9 h-9 rounded-full bg-cream/15 flex items-center justify-center shrink-0">
//         <span className="text-lg">🔥</span>
